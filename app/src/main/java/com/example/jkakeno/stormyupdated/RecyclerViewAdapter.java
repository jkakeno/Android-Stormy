...
//Gets the number of item view the adapter needs to hold depending on the size of array passed
    ...
    public int getItemCount() {
        return mDays !=null ? mDays.length : mHours.length;
    }
...
//Since the item view for list of days and list of hours differ except for temp label and icon image bind the remaining item views based on the data its passed in
...
//Get the summary for a particular day using the adapter position
                conditions = mDays[getAdapterPosition()].getSummary();
//If the data passed is hours
            } else {
//Store the time label on a variable
                label = mTimeLabel.getText().toString();
//Get the summary for a particular hour using the adapter position
                conditions = mHours[getAdapterPosition()].getSummary();
            }
